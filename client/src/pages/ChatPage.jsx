...
function ChatPage() {
  ...
  const { messages, getMessages, subcribeToMessages, unsubscribeFromMessages } =
    useMessageStore();
  ...
  useEffect(() => {
    if (authUser && id) {
      (async () => {
        await getMyMatches(); // Ensure matches are fetched first
        getMessages(id);      // Fetch messages for the selected match
        subcribeToMessages(); // Subscribe to message updates
      })();
    }
  ...
  }, [authUser, getMyMatches, getMessages, subcribeToMessages, unsubscribeFromMessages, id]);
      ...
      <div className="flex-grow flex flex-col p-4 md:p-6 lg:p-8 overflow-hidden max-w-4xl mx-auto w-full">
        <div className="flex items-center mb-4 bg-white rounded-lg shadow p-3">
          <img
            ...
            alt=""
            className="w-12 h-12 object-cover rounded-full mr-3 border-3 border-pink-300"
          />
